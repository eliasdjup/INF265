...
def grad_check(model, x, y, loss_fn, eps=10e-5):
    ...
    with torch.no_grad():
        ...
        if res_backprop > eps:
            print("RES BACKPROP", res_backprop)
            flag=False
    return flag
    ...
    for i_layer in range(1, model.L+1):
        ...
        for param_name in ['weight', 'bias']:
            ...
            if not param.is_leaf:
                ...
                res = False
    if res:
        print("All parameters seem correctly attached to the computational graph! :) ")
...
def check_gradients(model, backprop_fn, optimizer, loss_fn, x, y_true, n_epochs=5, eps=10e-5, verbose=False):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for i, (input, out_expected) in enumerate(zip(x, y_true)):
            ...
            grad_check_ok &= grad_check(model, input, out_expected, loss_fn,eps=eps)
            ...
            if i == (len(x)-1) and (epoch % 1 == 0):
                ...
                compare_autograd_ok &= compare_with_autograd(model, verbose=verbose)
                if compare_autograd_ok:
                    print("Gradients consistent with autograd's computations. :) ")
                if grad_check_ok:
                    print("Gradients consistent with finite differences computations. :) ")
...
def main_test(backprop_fn, model, eps=10e-5, verbose=False, data='toy'):
    ...
    # Training loop that compares our gradients with autograd's computations
    check_gradients(model, backprop_fn, optimizer, loss_fn, inputs, outputs, eps=eps, n_epochs=n_epochs, verbose=verbose)
    ...
    check_computational_graph(model)
